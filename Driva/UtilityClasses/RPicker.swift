...
let pickerAnimationDuration: TimeInterval = 0.0
...
class RPicker: NSObject {
    ...
    class func selectDate(title: String = "",
        ...
        if let currentController = APPDELEGATE.window?.currentController {
            ...
            components.year = +150
            let maxDate: NSDate = gregorian.date(byAdding: components as DateComponents, to: currentDate as Date as Date, options: NSCalendar.Options(rawValue: 0))! as NSDate as NSDate
...
//            toolBar.appendButton(buttonItem: toolBar.)
        }
